<x-layouts.app>
  <div class="relative">
    ...
    <div class="container flex flex-row items-center justify-between py-16">
    ...
    <div x-cloak x-data="{
            selectedSlug: '{{ request()->segment(2) ?? \App\Models\Smile::first()->slug }}',
            changeUrl(slug) {
                const newUrl = `/guluslerimiz/${slug}`;
      ...
      <div class="flex flex-col p-8 bg-white rounded-2xl lg:col-span-9">
        @foreach (\App\Models\Smile::all() as $item)
        <div x-show="selectedSlug==='{{ $item->slug }}'" class="space-y-10">
          ...
          @if ($item->content)
          <div class="max-w-full text-2xl font-light whitespace-pre-line  tracking-tighter prose text-text">@markdown($item->content)</div>
          @endif
        </div>
        @endforeach
      </div>
